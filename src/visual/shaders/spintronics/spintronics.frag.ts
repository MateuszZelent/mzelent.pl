...
uniform int uColorMap; // 0: hsl-cone, 1: zelent-prb, 2: racetrack, 3: chiral, 4: topological, 5: magnetization
...
//   -z (mz = -1) -> L = 0.0 -> BLACK (0, 0, 0)
//   +z (mz = +1) -> L = 1.0 -> WHITE (1, 1, 1)
//   in-plane (mz = 0) -> L = 0.5, S = 1.0 -> FULL SATURATION RAINBOW
...
void main() {
  ...
  if (uColorMap == 0) {
    // 0: MMPP HSL Color Cone (+z = white, -z = black, in-plane = continuous rainbow)
    float inPlaneAngle = atan(m.y, m.x);
    float h = mod(inPlaneAngle / 6.283185307, 1.0);
    if (h < 0.0) h += 1.0;
    float s = 1.0;
    float l = clamp((m.z + 1.0) * 0.5, 0.0, 1.0);
    baseColor = hsl2rgb_glsl(h, s, l);
  } else if (uColorMap == 1) {
    // 1: Zelent Publication Colormap (PRB/RRL)
    ...
    float t = clamp((m.z + 1.0) * 0.5, 0.0, 1.0);
    baseColor = mix(vec3(0.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0), t);
  }

  // Subtle concentric nanoscale lithography guide tracks
  float ringDist = fract(r / 0.5);
  float ringLine = smoothstep(0.98, 0.995, ringDist) + smoothstep(0.02, 0.005, ringDist);
  baseColor = mix(baseColor, vec3(0.5), ringLine * 0.04);
  ...
  // Soft diffuse shading maintaining pure colors and black/white contrast
  vec3 finalColor = baseColor * (0.92 + diff * 0.08);

  // Subtle micro-specular sheen
  finalColor += vec3(1.0) * spec * 0.05;

  // Crisp, elegant circular nanodot perimeter bevel (at r ~ 2.15)
  float bevelRim = smoothstep(2.12, 2.15, r) * smoothstep(2.18, 2.15, r);
  finalColor = mix(finalColor, vec3(0.35, 0.40, 0.45), bevelRim * 0.5);

  // Antialiased outer disk boundary falloff
  float diskMask = smoothstep(2.17, 2.14, r);
  finalColor *= diskMask;

  // Solid, vibrant nanodot disk with smooth color transition (opacity 0.98)
  gl_FragColor = vec4(finalColor, 0.98 * diskMask);
}
`;

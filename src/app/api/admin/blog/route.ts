...
async function parseBlogPostPayload(request: Request): Promise<BlogPost> {
  const contentType = request.headers.get("content-type") || "";

  if (contentType.includes("multipart/form-data")) {
    const formData = await request.formData();

    const id = (formData.get("id") as string)?.trim() || `post-${Date.now()}`;
    ...
    if (!imageUrl) {
      const existing = blogPostsData.find((p) => p.id === id);
      imageUrl = existing ? existing.imageUrl : "/images/blog/skyrmion-lattice-sim.webp";
    }
...
export async function POST(request: Request) {
  ...
  try {
    const newPost = await parseBlogPostPayload(request);
...
export async function PUT(request: Request) {
  ...
  try {
    const updatedPost = await parseBlogPostPayload(request);

    const exists = blogPostsData.some((p) => p.id === updatedPost.id);
    if (!exists) {
      return NextResponse.json({ error: `Post ${updatedPost.id} not found` }, { status: 404 });
    }

    const updated = blogPostsData.map((p) => (p.id === updatedPost.id ? updatedPost : p));
    persistBlogPosts(updated);

    return NextResponse.json({ success: true, post: updatedPost });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : "Validation or update error";
    return NextResponse.json({ error: message }, { status: 400 });
  }
}
